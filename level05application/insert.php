...
try {
    ...
    $dbOk = false;
    error_log('level05application/insert db load failed: ' . $e->getMessage());
...
$nicBlock = nic_normalize((string) ($p['student_nic'] ?? ''));
try {
    $blockMsg = l05_other_level_application_blocks($conn, $nicBlock, L05_APP_LEVEL);
    if ($blockMsg !== null) {
        http_response_code(422);
