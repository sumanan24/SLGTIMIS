...
class AdminController extends Controller {
    ...
    /**
     * Database Backup - Download full SQL file including data, triggers, routines, views, and events.
     */
    public function backupDb() {
        ...
        try {
            require_once BASE_PATH . '/helpers/BackupHelper.php';

            $dbName = DB_NAME;
            $sql = BackupHelper::tryMysqldump($dbName);
            if ($sql === null) {
            ...
            BackupHelper::beginBrowserDownload($filename, 'application/sql', strlen($sql));
            echo $sql;
            exit;
        } catch (Exception $e) {
    ...
    /**
     * File Backup - Download uploads and assets folders as a zip archive.
     */
    public function backupFiles() {
        ...
        try {
            ...
            $filename = 'files_backup_' . DB_NAME . '_' . date('Y-m-d_H-i-s') . '.zip';

            require_once BASE_PATH . '/core/ActivityLogger.php';
            $activityLogger = new ActivityLogger();
            $activityLogger->log('files_backup', 'File backup zip downloaded by admin', 'success');

            BackupHelper::streamFoldersZip(BackupHelper::defaultFileBackupDirs(), $filename);
        } catch (Exception $e) {
            $_SESSION['error'] = 'File backup failed: ' . $e->getMessage();
            $this->redirect('admin/users');
        }
    }
}
