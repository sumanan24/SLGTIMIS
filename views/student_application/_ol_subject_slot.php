<?php
/**
 * One O/L subject row: fixed main subjects (1–6) + basket dropdowns (7–9) + result.
 *
 ...
 * @var string $extraAttr Extra HTML attributes for controls (e.g. ' required')
...
/** @var array<string, array<string, mixed>> $olSlotMap */
...
$fixedSubject = isset($def['fixed_subject']) ? (string) $def['fixed_subject'] : '';
$slotSubjects = isset($def['subjects']) && is_array($def['subjects']) ? $def['subjects'] : [];
/** O/L results offered in the UI (legacy grades may still appear when editing older records). */
$gradeLetters = ['A', 'B', 'C', 'S', 'W'];

$hiddenVal = $rawName !== '' ? $rawName : $fixedSubject;
                    ...
                    <div class="app-ol-row app-exam-subj-cell h-100">
                        <div class="app-subject-block">
                            ...
                            <div class="app-subj-fields">
                                <input type="hidden" name="ol_subject_name_<?php echo htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); ?>" id="ol_subject_name_<?php echo htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); ?>" value="<?php echo htmlspecialchars($hiddenVal, ENT_QUOTES, 'UTF-8'); ?>" data-fixed-subject="<?php echo htmlspecialchars($fixedSubject, ENT_QUOTES, 'UTF-8'); ?>">
...
/* Basket slots — subject dropdown + result */
if ($variant === 'form') {
    ?>
                <div class="col-12 col-lg-6">
                    <div class="app-ol-row app-exam-subj-cell h-100">
                        <div class="app-subject-block">
                            ...
                            <div class="app-subj-fields">
                                ...
                                <select name="ol_subject_name_<?php echo htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); ?>" id="ol_subject_name_<?php echo htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); ?>" class="form-select form-select-sm app-subj-name app-exam-input-compact" aria-label="<?php echo htmlspecialchars($slotLabel, ENT_QUOTES, 'UTF-8'); ?>"<?php echo $extraAttr; ?>>
                                    ...
                                    <option value="<?php echo htmlspecialchars((string) $subj, ENT_QUOTES, 'UTF-8'); ?>"<?php echo $rawName === $subj ? ' selected' : ''; ?>><?php echo htmlspecialchars((string) $subj, ENT_QUOTES, 'UTF-8'); ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <div class="app-subj-mark-group">
                                    <label class="form-label app-subj-mark-label mb-0" for="ol_subject_<?php echo htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); ?>_marks">Result</label>
