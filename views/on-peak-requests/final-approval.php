...
<div class="container-fluid px-4 py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <div>
            ...
            <p class="text-muted mb-0">Approve or reject requests that have been approved by HOD</p>
        </div>
    </div>
    ...
    <?php if (empty($requests)): ?>
        <div class="alert alert-info">
            <i class="fas fa-info-circle me-2"></i>No pending requests for final approval.
        </div>
    <?php else: ?>
        <?php foreach ($requests as $request): ?>
            <div class="request-card p-4">
                <div class="approval-path mb-3">
                    <small class="text-muted d-block mb-1">Approval Status:</small>
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        <span class="badge bg-success">
                            <i class="fas fa-check me-1"></i>HOD Approved
                        </span>
                        <span class="small text-muted">
                            <?php if ($request['hod_approver_name']): ?>
                                by <?php echo htmlspecialchars($request['hod_approver_name']); ?>
                            <?php endif; ?>
                            <?php if ($request['hod_approval_date']): ?>
                                on <?php echo date('M d, Y', strtotime($request['hod_approval_date'])); ?>
                            <?php endif; ?>
                        </span>
                    </div>
                </div>
                            ...
                            <div class="fw-semibold">
                                <?php if (isset($request['is_hostel_student']) && $request['is_hostel_student'] == 1): ?>
                                    ...
                                    <?php if ($request['student_gender']): ?>
                                        <small class="text-muted d-block">(<?php echo htmlspecialchars($request['student_gender']); ?>)</small>
                                    <?php endif; ?>
                                <?php else: ?>
                ...
                <?php if (!empty($request['hod_comments'])): ?>
                    <div class="mb-3">
                        <small class="text-muted d-block mb-2">HOD Comments:</small>
                        <div class="p-3 bg-light border rounded"><?php echo nl2br(htmlspecialchars($request['hod_comments'])); ?></div>
                    </div>
                <?php endif; ?>
                
                <div class="mb-3">
                    <small class="text-muted d-block mb-2">HOD Approved:</small>
                    <div class="small"><?php echo $request['hod_approval_date'] ? date('F d, Y h:i A', strtotime($request['hod_approval_date'])) : 'N/A'; ?></div>
                </div>
                
                <form action="<?php echo APP_URL; ?>/on-peak-requests/final-approve" method="POST" class="approval-form">
